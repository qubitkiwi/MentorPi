...
class JoystickController(Node):
    ...
    def axes_callback(self, axes):
        ...
        if self.machine == 'MentorPi_Mecanum':
            ...
            twist.linear.x = val_map(axes['1'], 1, -1, -self.max_linear, self.max_linear)
            steering_angle = val_map(axes['2'], 1, -1, -math.radians(322/2000*180), math.radians(322/2000*180))
            if twist.linear.x == 0:
                twist.linear.z = 1.0
        ...
        # 例如，使用十字键来控制前进/后退的线速度
        if axes['hat_x'] == 1:  # 上
            twist.linear.x = 1.0  # 增加前进速度
        elif axes['hat_x'] == -1:  # 下
            twist.linear.x = -1.0  # 减少前进速度

        # 使用十字键的左右控制机器人旋转
        if axes['hat_y'] == 1:  # 右
            twist.linear.y = 1.0  # 增加旋转速度
        elif axes['hat_y'] == -1:  # 左
            twist.linear.y = -1.0  # 减少旋转速度        
